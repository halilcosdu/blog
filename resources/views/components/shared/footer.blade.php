    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
            ...
            <div class="md:col-span-4">
                ...
                <div class="mt-6 flex items-center gap-3 text-slate-400">
                    <!-- X (formerly Twitter) -->
                    <a href="#" class="w-9 h-9 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 grid place-items-center transition-colors" aria-label="X">
                    ...
                    <!-- YouTube -->
                    <a href="#" class="w-9 h-9 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 grid place-items-center transition-colors" aria-label="YouTube">
                    ...
                    <!-- GitHub -->
                    <a href="#" class="w-9 h-9 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 grid place-items-center transition-colors" aria-label="GitHub">
                    ...
                    <!-- Instagram -->
                    <a href="#" class="w-9 h-9 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 grid place-items-center transition-colors" aria-label="Instagram">
            ...
            <div class="md:col-span-4">
                ...
                <div id="footer-top-lessons" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    @foreach($topLessons->take(10) as $t)
                        <a href="/posts/{{ $t->slug }}" class="group rounded-xl border border-slate-800/80 bg-slate-800/40 hover:bg-orange-500/20 hover:border-orange-500/40 transition-colors">
                            <div class="aspect-video overflow-hidden rounded-t-xl">
                                <img src="{{ $t->featured_image }}" alt="{{ $t->title }}" class="w-full h-full object-cover">
                            </div>
                            <div class="p-3">
                                <div class="text-sm font-semibold text-slate-100 line-clamp-2 group-hover:text-white">{{ $t->title }}</div>
                                <div class="mt-1 text-[12px] text-slate-400">{{ $t->category?->name }} • {{ $t->reading_time }}</div>
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 items-center rounded-2xl border border-slate-800/80 bg-slate-800/40 p-6">
            ...
            <form class="flex gap-3">
                ...
                <button type="submit" class="px-5 py-3 rounded-xl bg-gradient-to-r from-red-600 to-orange-500 font-semibold hover:shadow-lg hover:shadow-red-500/20 transition">Subscribe</button>
            </form>
        </div>
